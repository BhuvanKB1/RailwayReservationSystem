const {Router} = require("express");
const bookingController  =require('../controllers/bookingControllers')
const isAuthenticated = require('../../middlewares/isAuthenticated');
const router =Router();

/**
 * @swagger
 * components:
 *   schemas:
 *     Book:
 *       type: object
 *       required:
 *         - booking_id
 *         - train_id
 *         - user_id
 *         - quantity
 *         - total_price
 *       properties:
 *         id:
 *           type: string
 *           description: The auto-generated id of the Book
 *         booking_id:
 *           type: string
 *           description: this is booking id
 *         booking_date:
 *           type: date
 *           description: this is booking date
 *         train_id:
 *           type: number
 *           description: This is train id
 *         user_id:
 *           type: string
 *           description: This is user id
 *         quantity:
 *           type: number
 *           description: The total tickets
 *         total_price:
 *           type: number
 *           description: This is total checkout price
 *       
 */


 /**
  * @swagger
  * tags:
  *   name: Book
  *   description: The Book managing API
  */



//router.get('/train/book',bookingController.book_get);




/**
 * @swagger
 * /train/book:
 *   post:
 *     summary: To Book Ticket
 *     tags: [Book]
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 *           schema:
 *             $ref: '#/components/schemas/Book'
 *     responses:
 *       200:
 *         description: Booking successful
 *         content:
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/Book'
 *       400:
 *         description: Booking not succesfull
 */


router.post('/train/book',isAuthenticated,bookingController.book_post);


/**
 * @swagger
 * /train/cancelticket/{id}:
 *   delete:
 *     summary: Cancel Booking
 *     tags: [Book]
 *     parameters:
 *       - in: path
 *         name: id
 *         schema:
 *           type: string
 *         required: true
 *         description: The booking id
 * 
 *     responses:
 *       200:
 *         description: The booking canceled
 *       400:
 *         description: The booking not canceled
 */





router.delete('train/cancelticket/:id',isAuthenticated,bookingController.book_delete);




module.exports =router;